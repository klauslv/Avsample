...
void Gles_play::playYUV(jobject surface) {
    ...
    /**
       ...
       */
    //对sample变量，使用函数glUniform1i和glUniform1iv进行设置
    ...
    if (!fp) {
        LOGD("open file s% fail", data_source);
        return;
    }
    ...
    while (!feof(fp)) {
        ...
        fread(buf[1], 1, width * height, fp);
        fread(buf[2], 1, width * height, fp);
...
void Gles_play::release() {
    if (display || windowSurface || context) {
        //销毁显示设备
        ...
        eglMakeCurrent(display, windowSurface, EGL_NO_SURFACE, context);
